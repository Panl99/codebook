...
> [Python3.8.1标准库参考中文手册.pdf](https://github.com/Panl99/codebook/tree/master/resources/static/doc/Python3.8.1标准库参考中文手册(The&ensp;Python&ensp;Library&ensp;Reference)&ensp;高清pdf版.pdf)
...
- [一、Python基础](#一、Python基础)
    ...
    - [4、文件、异常](#4、文件、异常)
- [二、数据分析](#二、数据分析)
    - [1、NumPy：N维数组表达基础库](#1、NumPy：N维数组表达基础库)
    - [2、Pandas：数据分析高层次应用库](#2、Pandas：数据分析高层次应用库)
    - [3、SciPy：数学、科学、工程计算](#3、SciPy：数学、科学、工程计算)
    - [4、数据可视化](#4、数据可视化)
    - [5、文本处理](#5、文本处理)
- [三、网络爬虫](#三、网络爬虫)
    - [1、Requests：网络爬虫功能库，页面爬取](#1、Requests：网络爬虫功能库，页面爬取)
    - [2、Scrapy：网络爬虫框架](#2、Scrapy：网络爬虫框架)
    - [3、pyspider：web页面爬取系统](#3、pyspider：web页面爬取系统)
- [四、Web解析](#四、Web解析)
    - [1、Beautiful Soup：HTML/XML解析库](#1、Beautiful Soup：HTML/XML解析库)
    - [2、Re：正则（标准库，不需要安装）](#2、Re：正则（标准库，不需要安装）)
    - [3、Python-Goose：提取文章类型的web页面](#3、Python-Goose：提取文章类型的web页面)
- [五、Web开发](#五、Web开发)
    - [1、Django：web应用框架，MTV模式](#1、Django：web应用框架，MTV模式)
    - [2、Flask：微框架，适用于仅几个页面](#2、Flask：微框架，适用于仅几个页面)
    - [3、pyramid：中等规模web应用框架](#3、pyramid：中等规模web应用框架)
- [六、机器学习](#六、机器学习)
...
#### 元组（tuple）
- **与列表类似(语法可参考列表)，不同之处在于元组的元素不能修改(但它可以包含可变的对象，如list列表)。**
- **元组写在小括号( )里，元素之间用逗号隔开**
```
# 0个元素元组，空元祖
tup1 = ()
