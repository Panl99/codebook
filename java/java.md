...
|树|[二叉树](#二叉树)|[红黑树](#红黑树)|[B Tree](#B-Tree)|[B+ Tree](#B+-Tree)||
| |[图](#图)|[位图Bitmap](#位图)||||
...
7. HashMap、ConcurrentHashMap、HashTable、TreeMap
...
12. 创建对象的4种方式：
    - （1）new 对象
    - （2）[反射机制](#创建对象的两种方式)
    - （3）clone()方法：继承Cloneable接口；重写Object类的clone()方法；在clone()方法中调用`super.clone()`。
    - （4）[反序列化](#序列化与反序列化)
