...
- 解决方案：
  - 临时锁：在访问key之前，通过SETNX设置一个短期key来锁住当前key的访问，访问结束后再删掉该短期key。
...
### 短结构
- 当列表、散列、有序集合的长度较短(或者体积较小)时，Redis可以使用**[压缩列表](#压缩列表)(ziplist)** 的紧凑存储方式来存储这些结构。
...
![](../resources/static/images/redis-ziplist.PNG)
<img src="../resources/static/images/redis-ziplist.PNG" />
...
- 更新的时候，先删除缓存，然后更新数据库。
    - 为什么是删除缓存，不是更新缓存呢？
        - 有些场景，缓存不只是数据库中取出的值，所以更新缓存的代价较高。
        - 频繁修改一个缓存设计的多个表，那么缓存会被频繁的更新，但是缓存不一定会被频繁的使用到。

- 问题：先修改数据库，再删除缓存。如果缓存删除失败，会导致数据库中是新数据，缓存中是老数据。
    - 解决：先删缓存，再修改数据库。如果删缓存成功，修改数据库失败，那么数据库中是旧数据，缓存中是空的，数据就不会不一致。
